...
class LicenseValidator {
    ...
    public void verify(PublicKey publicKey, int responseCode, String signedData, String signature) {
        ...
        switch (responseCode) {
            case 0:
            case 2:
                Policy.LicenseResponse limiterResponse = this.mDeviceLimiter.isDeviceAllowed(userId);
                handleResponse(limiterResponse, data);
                return;
            case 1:
                handleResponse(Policy.LicenseResponse.NOT_LICENSED, data);
                return;
            case 3:
            ...
            case 4:
                ...
                handleResponse(Policy.LicenseResponse.RETRY, data);
                return;
            case 5:
    ...
    private void handleResponse(Policy.LicenseResponse response, ResponseData rawData) {
        this.mPolicy.processServerResponse(response, rawData);
        if (this.mPolicy.allowAccess()) {
            this.mCallback.allow();
        } else if (response == Policy.LicenseResponse.NOT_LICENSED) {
            this.mCallback.dontAllow();
        } else {
            this.mCallback.applicationError(LicenseCheckerCallback.ApplicationErrorCode.NETWORK_ERROR);
        }
    }

    private void handleApplicationError(LicenseCheckerCallback.ApplicationErrorCode code) {
