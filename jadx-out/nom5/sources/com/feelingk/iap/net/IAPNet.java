...
public class IAPNet {
    ...
    protected static InitConfirm iapConnect(ServerInfo info, int telecom, String mcid, String min, String bpServerIP, int bpServerPort, String pID, String pTID, String pEncJumin, boolean useBpServer) {
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] Server connect - Start : iapConnect Wifi= " + isWifi);
        if (connect) {
            ...
            try {
                ...
                connect = true;
                CommonF.LOGGER.m2i(TAG, "[ DEBUG ] Server connect - Complete!!!");
                ...
                if (initConfirm.getResultCode() != 0) {
                    ...
                    int telecom2 = initConfirm.getResultCode();
                    if (telecom2 != 0) {
                        isInit = false;
                ...
                connect = false;
                CommonF.LOGGER.m2i(TAG, "[ Exception ] iapConnect() DUMP :" + e2.toString());
                initConfirm.setResultCode((byte) -3);
                initConfirm.SetUserMessage(CommonString.ERROR_NETWORK_CONNECT_CHECK_STRING);
                e2.printStackTrace();
            } catch (Exception e3) {
                CommonF.LOGGER.m2i(TAG, "[ Exception ] iapConnect() DUMP :" + e3.toString());
                connect = false;
                initConfirm.setResultCode((byte) -3);
    ...
    protected static int iapClose(boolean isResRelease) {
        CommonF.LOGGER.m0e(TAG, "[ DEBUG ] Socket Close!");
        try {
            if (inputStream != null) {
                inputStream.close();
            }
    ...
    protected static void iapSendInit(InitConfirm initConfirm, int pTelecom, String applicationID, String encJuminNumber, String MIN, String bpServerIP, int bpServerPort, String pID, String pTID, boolean useBpServer) {
        ...
        int offset;
        CommonF.LOGGER.m2i(TAG, "iapSendInit  Start() isInit=" + isInit);
        CommonF.LOGGER.m2i(TAG, "iapSendInit  Start() useBpServer =" + useBpServer);
        CommonF.LOGGER.m2i(TAG, "iapSendInit  Start() encJuminNumberCheck =" + (encJuminNumber != null ? 1 : 0));
        if (isInit) {
        ...
        int i2 = packet_len5 - 12;
        byte[] header = IAPNetworkUtil.iapMakePacketHeader(sHeaderPacket, i2);
    ...
    protected static InitConfirm iapReAuth(int pTelecom, String applicationID, String pJuminNumber, String MDN, String bpServerIP, int bpServerPort, String pID, String pTID) {
        ...
        int pTelecom2 = initConfirm.getResultCode();
        if (pTelecom2 == 0) {
            byte[] recv = iapReceive(initConfirm);
            if (initConfirm.getResultCode() != 0) {
                initConfirm.SetUserMessage(CommonString.ERROR_NETWORK_SEND_RECV_CHECK_STRING);
    ...
    protected static synchronized byte iapWrite(byte[] sendBuf) {
        byte b;
        CommonF.LOGGER.m0e(TAG, "[ DEBUG ] iapWrite connect(" + connect + ")");
        if (!connect) {
            b = -2;
        } else {
            ...
            try {
                try {
                    ...
                    outputStream.flush();
                    CommonF.LOGGER.m0e(TAG, "[ DEBUG ] iapWrite End  ");
                    b = 0;
                } catch (SocketTimeoutException e) {
                    CommonF.LOGGER.m2i(TAG, "[ Exception ] iapWrite() " + e);
                    e.printStackTrace();
                    b = Defines.IAP_GATEWAY_RESPONSE.IAP_ERR_SEND_TIMEOUT_FAIL;
                }
            } catch (IOException e2) {
                CommonF.LOGGER.m2i(TAG, "[ Exception ] iapWrite() " + e2);
                b = -4;
            }
        }
        return b;
    ...
    private static synchronized byte[] iapReceive(Confirm confirm) {
        byte[] bArr;
        CommonF.LOGGER.m0e(TAG, "[ DEBUG ] iapReceive() Header Start (" + isInit + ")");
        if (!isInit) {
            ...
            try {
                ...
                if (datalength.length() > 0) {
                    ...
                    if (23 != data[data_len - 1]) {
                        CommonF.LOGGER.m0e(TAG, "[ DEBUG ] IAP_ERR_INVALIDPARITY =");
                        ...
                        CommonF.LOGGER.m0e(TAG, "[ DEBUG ] iapReceive() Success!!  ");
                        bArr = returnData;
                    }
                } else {
                    confirm.setResultCode((byte) -5);
                ...
                CommonF.LOGGER.m2i(TAG, "[ Exception ] iapReceive() " + e);
                confirm.setResultCode(Defines.IAP_GATEWAY_RESPONSE.IAP_ERR_RECV_TIMEOUT_FAIL);
                bArr = null;
            } catch (Exception e2) {
                CommonF.LOGGER.m2i(TAG, "[ Exception ] iapReceive() " + e2);
                confirm.setResultCode((byte) -5);
                bArr = null;
            }
        }
    ...
    protected static InitConfirm iapConnectBP(ServerInfo info, int telecom, String mcid, String min, String bpServerIP, int bpServerPort) {
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapConnectBP - Start! isWifi= " + isWifi);
        if (connectBP) {
            ...
            try {
                ...
                connectBP = true;
                CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapConnectBP - Connection Complete!");
                ...
                CommonF.LOGGER.m2i(TAG, "[ Exception ] iapConnectBP() " + e);
                connectBP = false;
    ...
    protected static void iapSendInitBP(InitConfirm init, int pTelecom, String applicationID, String MIN, byte[] ip, int port) {
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendInitBP()  __ Start");
        int packet_len = 12 + 1;
        int packet_len2 = packet_len + 8 + 1 + 10 + 11 + 1;
        if (ip != null) {
            int packet_len3 = ip.length;
            packet_len2 = packet_len3 + 44;
        }
        int packet_len4 = packet_len2 + 7 + 10;
        if (pTelecom == 2 || pTelecom == 3) {
            packet_len4 = packet_len4 + 2 + 1 + Defines.IAP_SENDBPDATA_PROTOCOL.length();
        }
        ...
        byte[] header = IAPNetworkUtil.iapMakePacketHeader("IP", packet_len5 - 12);
        System.arraycopy(header, 0, packetBytes, 0, header.length);
        int offset = 0 + 12;
    ...
    protected static synchronized byte iapWriteBP(byte[] sendBuf) {
        ...
        if (!connectBP) {
            ...
            try {
                ...
                CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapWriteBP() " + e);
                b = -4;
            }
        }
        return b;
    ...
    protected static byte[] iapSendDataBP(byte[] data) {
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendDataBP()");
    ...
    private static synchronized byte[] iapReceiveBP(Confirm c) {
        ...
        try {
            ...
            CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapReceiveBP() Header Receive Complete!");
            String datalength = new String(header, 2, 10).trim();
            if (datalength.length() > 0) {
                int data_len = Integer.parseInt(datalength);
                CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapReceiveBP() Data Length: " + data_len);
                byte[] data = new byte[data_len];
                int receivedBytes2 = 0;
                while (receivedBytes2 < data_len) {
            ...
            CommonF.LOGGER.m2i(TAG, "[ Exception ] iapReceiveBP() " + e);
            c.setResultCode((byte) -5);
            bArr = null;
        }
        return bArr;
    ...
    protected static byte[] iapSendData(byte[] data) {
        ...
        CommonF.LOGGER.m0e(TAG, "[ DEBUG ] iapSendData!!!!() Start ~~~~~~~~~~~~~~~~~~~~");
        int packet_len = data.length + 12 + 1;
        byte[] packetBytes = new byte[packet_len];
        for (int i = 0; i < packet_len; i++) {
            packetBytes[i] = 0;
        ...
        CommonF.LOGGER.m0e(TAG, "[ DEBUG ] iapSendData() End~~~~~~~~~~~~~~~~~~~~");
    ...
    protected static MsgConfirm iapSendItemQuery(String pID, String pName, String pTID, String pBPInfo) {
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendItemQuery() PID=" + pID);
        int packet_len2 = 12 + 10;
        int packet_len3 = packet_len2 + 1;
        if (pName != null) {
            packet_len3 = pName.getBytes().length + 23;
            headerParam2 = "EP";
        }
        if (pTID != null) {
            int packet_len4 = packet_len3 + 100 + 1;
            headerParam = "NP";
            packet_len = packet_len4;
        } else {
            int packet_len5 = packet_len3;
            headerParam = headerParam2;
            packet_len = packet_len5;
        }
        if (pBPInfo != null) {
            packet_len += pBPInfo.getBytes().length;
        }
        int packet_len6 = packet_len + 1;
        byte[] packetBytes = new byte[packet_len6];
        for (int i = 0; i < packet_len6; i++) {
            packetBytes[i] = 0;
        }
        byte[] header = IAPNetworkUtil.iapMakePacketHeader(headerParam, packet_len6 - 12);
    ...
    protected static ItemInfoConfirm iapSendItemInfoQuery(String pID) {
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendItemInfoQuery()");
    ...
    protected static MsgConfirm iapSendItemPurchase(String pID, String pName, Boolean bTCash, String TID, String BPInfo, Boolean bUseBPProtocol) {
        ...
        int packet_len;
        MsgConfirm msgConfirm = new MsgConfirm();
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendItemPurchase() bUseBPProtocol = " + bUseBPProtocol + "  START !!!!!!!!");
        int packet_len2 = 12 + 10;
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendItemPurchase()  bTCash=" + bTCash + "// PName=" + (pName == null ? 0 : 1));
        byte[] header = IAPNetworkUtil.iapMakePacketHeader(headerParam, packet_len6 - 12);
        System.arraycopy(header, 0, packetBytes, 0, header.length);
        int offset = 0 + 12;
        byte[] byte_pID = String.valueOf(pID).getBytes();
    ...
    protected static MsgConfirm iapSendItemPurchaseByDanal(String mdn, String pID, String pName, int pCarrier, String TID, String BPInfo, boolean bUseTCash, String encJumin) {
        ...
        if (pName != null) {
            int packet_len4 = pName.length();
            packet_len = packet_len4 + 13;
        } else {
        ...
        if (TID == null && BPInfo == null) {
            ...
            int packet_len5 = packet_len + 100 + 1;
            if (BPInfo != null) {
                useXPProtocol = true;
                packet_len2 = packet_len5 + BPInfo.length();
                headerParam = "XP";
            } else {
                useXPProtocol = true;
                packet_len2 = packet_len5;
                headerParam = "XP";
            }
        }
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendItemPurchaseByDanal() STart!! " + headerParam);
        int packet_len6 = packet_len2 + 10 + 2 + 11 + 1 + encJumin.length() + 1 + 1;
        if (pCarrier == 2) {
        ...
        CommonF.LOGGER.m0e("DEBUG", "Jumin=" + encJumin.length() + "  // Value = " + encJumin);
        if (bUseTCash) {
            int i2 = offset7 + 1;
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendItemPurchaseByDanal()  End !!!!!!!!");
        return itemPurchaseDanalCfm;
    }
    ...
    protected static ItemWholeAuthConfirm iapSendItemWholeAuth() {
        ...
        CommonF.LOGGER.m2i(TAG, "[ DEBUG ] iapSendItemWholeAuth()");
        int packet_len = 12 + 1;
        byte[] packetBytes = new byte[packet_len];
        for (int i = 0; i < packet_len; i++) {
            packetBytes[i] = 0;
